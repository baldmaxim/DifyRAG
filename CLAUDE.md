...
- Одна строка, кратко и по делу; без тела коммита, если не просят.
...
```text
Добавил поиск через RAG, внешний API, роли и ограничение частоты запросов
Подключил Dify: загрузка документов, интеграции и фоновый обработчик
Поправил схему развёртывания: сначала всё на тестовом GPU-хосте, потом переезд на сервер
```
